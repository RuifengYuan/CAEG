...
class DynamicRagModel(DynamicRagPreTrainedModel):
    ...
    def forward(
        ...
        if input_emb != None:
            emb_layer = self.generator.model.encoder.embed_tokens
            pos_layer = self.generator.model.encoder.embed_positions
            
            context_input_emb=emb_layer(context_input_ids)
            
            input_emb=self.hidden_mlp(input_emb)
            input_emb=input_emb.unsqueeze(1)
            
            context_input_emb=torch.cat([input_emb, context_input_emb],1)
            context_input_emb+=pos_layer(context_input_emb.size()[:-1])
            context_attention_mask=torch.cat([torch.full((context_input_emb.size()[0], context_input_emb.size()[1]-context_attention_mask.size()[1]), 1 ,dtype=torch.long).cuda(),context_attention_mask],1)
                
            
    
            gen_outputs = self.generator(
                #input_ids=context_input_ids,
                inputs_embeds=context_input_emb,
...
class DynamicRagForGeneration(DynamicRagPreTrainedModel):
    ...
    def generate(
        ...
        if input_emb != None:
            emb_layer = self.rag.generator.model.encoder.embed_tokens
            pos_layer = self.rag.generator.model.encoder.embed_positions
            
            context_input_emb=emb_layer(context_input_ids)
            
            input_emb=self.rag.hidden_mlp(input_emb)
            input_emb=input_emb.unsqueeze(1)
            ...
            encoder = self.rag.generator.get_encoder()
            encoder_outputs = encoder(inputs_embeds=context_input_emb, attention_mask=context_attention_mask, return_dict=True)
        else:
            encoder = self.rag.generator.get_encoder()
            encoder_outputs = encoder(input_ids=context_input_ids, attention_mask=context_attention_mask, return_dict=True)
