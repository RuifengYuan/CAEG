...
class Train(object):
    ...
    def __init__(self, config):
        ...
        if self.config.mid_start == 0:
            ...
            self.generator.load_state_dict(torch.load('save_model/best-generator.ckpt', map_location=lambda storage, loc: storage),strict=False)
    ...
    def train_one_batch(self):
        ...
        for idx in gold_top:
            min_id=max(0,idx-1)
            max_id=min(len(all_source)-1,idx+1)
            source.append(all_source[min_id]+" // "+all_source[idx]+" // "+all_source[max_id])
        for idx in pred_top:
            ...
            with open(self.gold_path, 'w', encoding='utf-8') as save_gold:
                    ...
                    for i in tqdm(range(int(test_num)), desc='testing'):            
                        ...
                        #for idx in gold_top:
                            #source.append(all_source[idx])
                        for idx in pred_top:
                            minmin_id=max(0,idx-2)
                            min_id=max(0,idx-1)
                            max_id=min(len(all_source)-1,idx+1)
                            maxmax_id=min(len(all_source)-1,idx+2)
                            source.append(all_source[min_id]+" // "+all_source[idx]+" // "+all_source[max_id])
