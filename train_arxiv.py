...
def data_loader_from_ext_to_abs(text, summary, max_source,max_target, tokenizer, pad_token_id, query='', prompt=[],global_prompt=[],global_sent=[], max_sent=25, context='c'):
    ...
    for turn_id in range(len(text)):
        ...
        if contextualized_turn != '':
            if context == 'c':
                src_texts=contextualized_turn
            if context == 'c+l':
                src_texts=" || ".join(prompt[turn_id]) + " // " + contextualized_turn                
            if context == 'c+l+g':
                src_texts=" || ".join(prompt[turn_id]) + " // " + " \\ ".join(global_prompt[turn_id]) + " // "  + contextualized_turn  
            if context == 'c+g':
...
class Train(object):
    ...
    def __init__(self, config):
        ...
        self.dataloader=data_loader('train', self.config, self.tokenizer_abs, 'abstractive', load_qmsum=0, load_arxiv=1)

        self.extractor=bart_extractor(self.config, 'save_model/best-retriever_arxiv.ckpt')   
        self.extractor.cuda()
        ...
        if self.config.mid_start == 0:
            ...
            self.generator = DynamicRagForGeneration.from_pretrained(self.config.pretrained_model_abs,
                                                                     n_docs=self.config.ndoc,
                                                                     gradient_checkpointing=True)
            self.generator.load_state_dict(torch.load('save_model/best-generator_arxiv.ckpt', map_location=lambda storage, loc: storage),strict=True)
    ...
    def train_one_batch(self):
        ...
        try:
            ...
            print('load data fail during the training')
            return 0,0
        ...
        try:
            ...
            print('training fail')
            return 0,0            
        seq_loss = generator_outputs.loss/(labels.size()[1])
        #seq_loss = generator_outputs.loss/self.config.true_batch_size
        seq_loss.backward()
    ...
    def do_val(self, val_num):
        ...
        data_loader_val=data_loader('val', self.config, self.tokenizer_abs, 'abstractive', load_qmsum=0, load_arxiv=1)
...
class Test(object):
    ...
    def test(self,test_num=7000):
        self.raw_rouge=Rouge()
        self.generator.eval()
        self.extractor.eval()
        
        data_loader_val=data_loader('test', self.config, self.tokenizer_ext, 'abstractive', load_qmsum=0, load_arxiv=1)
            ...
            with open(self.gold_path, 'w', encoding='utf-8') as save_gold:
                #with open(self.source_path, 'w', encoding='utf-8') as save_source:
                    ...
                    for i in tqdm(range(int(test_num)), desc='testing'):       
                        ...
                        #all_score=score_argumentation(all_score, all_hidden, 6)
...
def argLoader():
    ...
    parser.add_argument('--max_summary', type=int, default=400)

    parser.add_argument('--max_article', type=int, default=80)    
    ...
    parser.add_argument('--buffer_size', type=int, default=80)      
    ...
    parser.add_argument('--max_dec_steps', type=int, default=400)
    
    parser.add_argument('--min_dec_steps', type=int, default=200)
